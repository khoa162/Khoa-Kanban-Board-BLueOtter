...
import { addCards, updateCard } from '../../store/Actions';
import { 
  Container, 
  Input, 
  Button,
...
const Modal: React.FC<ModalProps> = ({visible}) => {
  ...
  const [cardCategory, setCardCategory] = useState<ICategory>(selectedCard?.category || ICategory.iOS);
  const [errorMessage, setErrorMessage] = useState<string | undefined>();
  const { selectedColumn } = useAppSelector((state => state.columns));
  ...
  useEffect(() => {
    ...
    setCardCategory(selectedCard?.category || ICategory.iOS);
  ...
  const handleSave = () => {
    ...
    if(!selectedCard?.id){
      ...
      dispatch(updateColumns({newCard}))
      ...
      <ModalContent>
        ...
        <CategoriesContainer>
          {Object.values(ICategory).map(category => (
            <LabelContainer key = {uuidv4()} color={getCategoryBackgroundColor(theme, category)}>
              <label>
                <input 
                  type='radio' 
                  name={category} 
